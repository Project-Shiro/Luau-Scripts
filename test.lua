...
local uiSettings = {
	editorInit = "print(\"I love funny cat memees\");",
...
local function updateSettings(key, value)
	...
	pcall(_writefile, "LettuceUi/settings.json", jsonEncode(httpService, uiSettings));
end
...
local function loadMainUI()
	...
	local main = create("Frame", { 
		...
		create("Frame", { 
			...
			create("ImageLabel", { 
				...
				BackgroundColor3 = Color3.fromHex("ffffff"), 
				BackgroundTransparency = 1, 
				Image = "rbxassetid://99551689533967", 
	...
	do
		...
		do
			...
			right.executeBox.MouseButton1Click:Connect(function()
				local func = loadstring(content.Text)
				if func then
					func()
				end
			end);
			...
			--	local func = loadstring(_getclipboard())
			--	if func then
			--		func()
			--	end
			--end);
			...
			right.loadClip.MouseButton1Click:Connect(function()
				...
				if #text > 16384 then
					warn("Clipboard Text is too large to be loaded to the editor. Execute directly using \"Execute Clipboard\".");
				else
					content.Text = text;
				end
			end);
		...
		do
			...
			local function setIcon(icon, path)
				...
				if res and icon.Parent.Parent == scroll then
					local filePath = string.format("LettuceUi/search/%s.%s", fileName, ext);
			...
			search.MouseButton1Click:Connect(function()
				if isSearching == false then
					...
					if _isfolder("LettuceUi/search") == false then
						_makefolder("LettuceUi/search");
					end
					local files = _listfiles("LettuceUi/search");
					for i = 1, #files do
						_delfile(files[i]);
					end
					calculateCellSize();
    ...
    do
		...
		do
			local typeToInfo = {
				MessageOutput = {
					colour = "#FFFFFF", -- White for general output
					prefix = "Output"
				},
				MessageInfo = {
					colour = "#2f6cd6", -- Blue for info
					prefix = "Info"
				},
				MessageWarning = {
					colour = "#d49d37", -- Yellow for warning
					prefix = "Warning"
				},
				MessageError = {
					colour = "#d44437", -- Red for error
					prefix = "Error"
				}
			};
			...
			local function appendConsole(msg, msgType)
				...
				content.Text ..= (content.Text == "" and "" or "\n") .. string.format("<font color=\"%s\" size=\"%d\">%s</font>", info.colour, content.TextSize, msg);
			end
	...
	--rconsoleprint("Lettuce Loaded Successfully");
    --print("pssstt ui loaded!!")
    _G.abcdnousenotify("Lettuce | Info", "Lettuce UI Loaded Successfully! Press Insert To Open!", 3)
end;
